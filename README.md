...
установленным PlatformIO, перед сборкой выбираем окружение `env:Falcon_dfu` для прошивки по DFU, `env:Falcon_stlink`
...
Также может быть использован индуктивный выключатель NPN с нормально замкнутым контактом LJ12A3-4-ZAX. [Пример подключения концевых выключателей](/Hardware/LIMIT_SWITCH.pdf).
